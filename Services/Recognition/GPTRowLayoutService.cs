...
namespace UpRestEye3.Services.Recognition
{
    ...
    public interface IGPTRowLayoutService
    {
        Task<SimplePageOfRows> LayoutParsingByLLM(SimplePageOfWords invoicePageText, InvoiceDTO currentInvoice);

    }

    public class GPTRowLayoutService : IGPTRowLayoutService
    {

        private readonly GPTRowsLayoutEnvironment _env;

        public GPTRowLayoutService()
        {
            _env = new GPTRowsLayoutEnvironment();
        }
       
        public async Task<SimplePageOfRows> LayoutParsingByLLM(SimplePageOfWords invoicePageText, InvoiceDTO currentInvoice)
        {
            try
            {
        ...
        private SimplePageOfRows ResponseInvoiceParsing(string responseContent)
        {
            try
            {
                // Разбор JSON-ответа
                    ...
                    contentElement.ValueKind == JsonValueKind.String)
                {
                    ...
                    if (rootContent.TryGetProperty("Rows", out contentElement))
                    {
                        ...
                        var tablesDataDocument = tablesDataJsonDocument.Deserialize<SimplePageOfRows>(options);
